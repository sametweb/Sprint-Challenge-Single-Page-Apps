...
> Answer: In traditional server-side routing method, when users click a link, the browser sends a new request to the server to retrieve information along with all other components that were downloaded before: header, footer, menus, sidebars of the website. This is not efficient, puts unnecessary burden on servers and takes longer for user to reach the information they request. Client-side routing method on the other hand, connects to the server and retrieves the HTML, CSS, and JavaScript of the website for once in the first time. After that, every connection is made to the server through AJAX withour reloading the whole page. After that point, browser's history API is not used and React Router enables to move among the screens and does the routing job for us.
